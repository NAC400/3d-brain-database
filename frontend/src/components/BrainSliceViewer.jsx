...
function BrainSlice({ sliceNumber, opacity = 1, position = [0, 0, 0] }) {
  // Try to load the texture, with fallback handling
  const [textureError, setTextureError] = useState(false);
  
  let texture;
  try {
    // Try to load NIH Visible Human data first
    texture = useTexture(`/assets/models/brain/visible-human-male/png/male_${sliceNumber}.png`);
  } catch (error) {
  ...
  const displayTexture = textureError ? createFallbackTexture() : texture;
  ...
  return (
    <div className="brain-slice-viewer" style={{ width: '100%', height: '600px', position: 'relative' }}>
      ...
      <div style={{ 
        ...
        {!texture && " Currently showing placeholder data - add NIH Visible Human PNG files to see real brain slices."}
      </div>
    </div>
  );
} 